...
/**
 * CKEditor text editor plugin base class.
 *
 * This is a base class for CKEditor plugins implemented within Moodle. These
 * plugins can optionally provide new buttons/plugins within CKEditor itself,
 * or configure the CKEditor options.
 *
 ...
 */
abstract class editor_ckeditor_plugin {
    ...
    protected function load_config() {
        if (!isset($this->config)) {
            $name = $this->get_name();
            $this->config = get_config("ckeditor_$name");
        }
    }
    ...
    public function set_config($name, $value) {
        ...
        set_config($name, $value, "ckeditor_$pluginname");
    }

    /**
     * Returns name of this ckeditor plugin.
     * @return string
     */
    public function get_name() {
        // All class names start with "ckeditor_".
        $words = explode('_', get_class($this), 2);
        return $words[1];
    }

    /**
     * Adjusts CKEditor init parameters for this plugin.
     *
     * Subclasses must implement this function in order to carry out changes
     * to the CKEditor settings.
     *
     * @param array $params CKEditor init parameters array
    ...
    /**
     * Counts the number of rows in CKEditor editor (row numbering starts with 1)
     *
     * @param array $params CKEditor init parameters array
    ...
    /**
     * Adds a JavaScript plugin into CKEditor. Note that adding a plugin does
     * not by itself add a button; you must do both.
     *
     * If you leave $pluginname blank (default) it uses the folder name.
     *
     * @param array $params CKEditor init parameters array
     * @param string $pluginname Identifier for plugin within CKEditor
     * @param string $jsfile Name of JS file (within plugin 'ckeditor' directory)
     */
    protected function add_js_plugin(&$params, $pluginname='', $jsfile='editor_plugin.js') {
        ...
        // Add special param that causes Moodle CKEditor init to load the plugin.
        ...
        $jsurl = $this->get_ckeditor_file_url($jsfile, false);
        $params['moodle_init_plugins'] .= $pluginname . ':' . $jsurl;
    }

    /**
     * Returns URL to files in the CKEditor folder within this plugin, suitable
     ...
     */
    public function get_ckeditor_file_url($file='', $absolute=true) {
        global $CFG;
        ...
        // Calculate the JS url (relative to the CKEditor plugins folder - using
        ...
        if ($CFG->slasharguments) {
            ...
            $jsurl = $this->plugin . '/ckeditor/' . $file;
        }

        if ($absolute) {
            $jsurl = $CFG->wwwroot . '/lib/editor/ckeditor/plugins/' . $jsurl;
        }
    ...
    protected function get_version() {
        ...
        $plugin = new stdClass;
        require($CFG->dirroot . '/lib/editor/ckeditor/plugins/' . $this->plugin . '/version.php');
        return $plugin->version;
    }

    /**
     * Calls all available plugins to adjust the CKEditor init parameters.
     *
     * @param array $params CKEditor init parameters array
     * @param context $context Context where editor is being shown
     * @param array $options Options for this editor
    ...
    public static function all_update_init_params(array &$params,
        ...
        $plugins = core_component::get_plugin_list('ckeditor');
        ...
        foreach ($plugins as $plugin => $dir) {
            ...
            $classname = 'ckeditor_' . $plugin;
            $pluginobjects[] = new $classname($plugin);
        }

        // Sort plugins by sort order and name.
        usort($pluginobjects, array('editor_ckeditor_plugin', 'compare_plugins'));
     ...
     * @return editor_ckeditor_plugin Plugin object
     */
    public static function get($plugin) {
        $dir = core_component::get_component_directory('ckeditor_' . $plugin);
        require_once($dir . '/lib.php');
        $classname = 'ckeditor_' . $plugin;
     ...
     * @param editor_ckeditor_plugin $a
     * @param editor_ckeditor_plugin $b
     * @return Negative number if $a is before $b
     */
    public static function compare_plugins(editor_ckeditor_plugin $a, editor_ckeditor_plugin $b) {
